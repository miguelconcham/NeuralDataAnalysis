...
for j= [1 2 3 4 5  7 8]
    ...
    plot(pctl_ranges+2.5,(stack_lm_pow_structure(j).speed_play_matrix(:,1)-stack_lm_pow_structure(j).speed_play_matrix(:,2))/std_pow, 'k:')
    speed_stack = [speed_stack;((stack_lm_pow_structure(j).speed_play_matrix(:,1)-stack_lm_pow_structure(j).speed_play_matrix(:,2))')/std_pow];
...
plot([0 100], [0 0], 'k')
for j= [1 2 3 4 5  7 8]
    plot(pctl_ranges+2.5,(stack_lm_pow_structure(j).acceleration_play_matrix(:,1)-stack_lm_pow_structure(j).acceleration_play_matrix(:,2))/std_pow, 'k:')
    acceleration_Stack = [acceleration_Stack;((stack_lm_pow_structure(j).acceleration_play_matrix(:,1)-stack_lm_pow_structure(j).acceleration_play_matrix(:,2))')/std_pow];
    animal_names = [animal_names;stack_lm_pow_structure(j).animal];
end

hold on
[~, ~, ci] = ttest(acceleration_Stack);
no_nan = ~isnan(sum(ci));
fill([pctl_ranges(no_nan)+2.5 fliplr(pctl_ranges(no_nan)+2.5)],[ci(1,no_nan) fliplr(ci(2,no_nan))], 'r', 'FaceAlpha',.5, 'EdgeColor','none')
plot(pctl_ranges+2.5, mean(acceleration_Stack), 'r', 'LineWidth',2)
xlabel('Acceleration')
ylabel('Pow difference')
ylim([-1 1])
