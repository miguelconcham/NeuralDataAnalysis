function [regressors, theta_matrix, rep_beh_regressor, partner_beh_regressor]  = compute_regressors_offset(psth_struct, prepost_intervals, repeated_animal)
% COMPUTE_REGRESSORS_OFFSET
% Like compute_regressors but aligns to BOUT OFFSET instead of onset.
%
% Inputs:
...
%   repeated_animal    - string, ID of recorded animal
%
% Outputs:
%   regressors   - [n_bouts x n_bins] matrix, 1 = in-bout, 0 = peri-bout, NaN = overlapping intervals
%   theta_matrix - [n_bouts x n_bins] matrix of theta power
%   rep_beh_regressor     - [n_bouts x n_bins] ramp for recorded animal behaviors
%   partner_beh_regressor - [n_bouts x n_bins] ramp for partner behaviors

%% Parameters
pre_bout  = prepost_intervals(1);
post_bout = prepost_intervals(2);
hist_range = psth_struct.hist_range;      % e.g., [-20 20]
bin_size = diff(hist_range) / (size(psth_struct.play_bout_offset,2) - 1);  
time_bins = linspace(hist_range(1), hist_range(2), size(psth_struct.play_bout_offset,2));
...
for i = 1:n_bouts
    ...
    % Absolute peri-bout limits relative to OFFSET
    pre_limit  = max(bout_end + pre_bout, bout_end - 0.5*(bout_end - prev_end));
    post_limit = min(bout_end + post_bout, bout_end + 0.5*(next_start - bout_end));

    % Relative to bout offset (0 at offset)
    rel_pre_start  = max(pre_limit - bout_end, hist_range(1)); % negative
    rel_post_end   = min(post_limit - bout_end, hist_range(2)); % positive
    rel_bout_start = max(bout_start - bout_end, hist_range(1)); % negative
    rel_bout_end   = 0;  % ends at 0
    ...
    theta_matrix(i,:) = psth_struct.play_bout_offset(i,:);
    ...
    % Initialize regressors for behaviors
    ...
    if ~isnan(first_rep_beh_start)
        rel_beh_bin = find(time_bins >= (first_rep_beh_start - bout_end), 1, 'first');
    ...
    if ~isnan(first_partner_beh_start)
        rel_beh_bin = find(time_bins >= (first_partner_beh_start - bout_end), 1, 'first');
        ramp_end_bin = min(rel_beh_bin + ramp_bins - 1, peri_idx(end));
        ramp_bins_indices = rel_beh_bin:ramp_end_bin;
        n_ramp = numel(ramp_bins_indices);
        x = linspace(0, pi, n_ramp);
