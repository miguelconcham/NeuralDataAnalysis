...
print(gcf,'-vector','-dsvg',[figure_dir,'\hmm with play vs wo play timewrapped.svg'])
...
[cluster_pvals, clusters, cluster_stats, perm_max_stats] = cluster_perm_test(matrix2plot_with_play(:,time_index), matrix2plot_without_play(:,time_index), alpha, 1000);

save([saving_folder,'\cluster_pvals_hmm_stats.mat'],'cluster_pvals','clusters','cluster_stats','perm_max_stats', 'matrix2plot_with_play', 'matrix2plot_without_play','alpha', 'wrapped_time','time_index','relevan_time')

%%


data1 = matrix2plot_with_play(:,time_index);
data2 =  matrix2plot_without_play(:,time_index);
    mean_diff = mean(data1,1, 'omitmissing') - mean(data2,1, 'omitmissing');
