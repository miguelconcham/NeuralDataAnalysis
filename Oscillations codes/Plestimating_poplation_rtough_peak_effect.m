...
session_name = all_neurons_TD.session,session_list;
...
save([saving_folder,'\all_neurons_comb_results.mat'],'comb_results', 'unique_sessions', 'valid_combos' ,'accepted_combinations','amount_per_type','com_psth_results','time2use', 'labels');
%%
saving_folder = '\\experimentfs.bccn-berlin.pri\experiment\PlayNeuralData\NPX-OPTO PLAY NMM\PlayBout Analysis\DataSets\Analysis results\Theta psth';
...
figure
plot(squeeze(mean(comb_results(:,indexes,1),2, 'omitmissing')),squeeze(mean(comb_results(:,~indexes,3),2, 'omitmissing')), '.')
hold on
plot([-2 2],[-2 2], 'k')
axis([-2 2 -2 2])
...
for j=1:numel(what2plot)
    subplot(3,6,j)
    com_psth_results_means = squeeze(mean(com_psth_results(:,what2plot(j),2,:),2, 'omitmissing'));
...
com_psth_results_means = squeeze(mean(com_psth_results(:,what2plot(j),3,:),2, 'omitmissing'));
pctl2plot =  prctile(com_psth_results_means,[5 95]);
    fill([time2use fliplr(time2use)], [pctl2plot(1,:) fliplr(pctl2plot(2,:))],'r', 'FaceAlpha',face_alpha, 'EdgeColor','none')
    hold on
    pctl2plot =  prctile(com_psth_results_means,[10 90]);
