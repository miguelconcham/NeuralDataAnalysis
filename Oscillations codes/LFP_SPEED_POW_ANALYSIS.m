function stack_lm_pow_structure = LFP_SPEED_POW_ANALYSIS(current_dir)
...
for ch_n=1:numel(mid_PAG_channel)
    ...
    spect_time                     = spect_time+min_time2analysis(pt);
    ...
    % speed_stack             = linspace(floor(min(spect_t_speed)), ceil(max(spect_t_speed)), n_stacks);
    speed_stack              = prctile(spect_t_speed(signal_in_range), 0:5:100);
    % acceleration_stack      = linspace(floor(min(spect_t_acceleration)), ceil(max(spect_t_acceleration)), n_stacks);
    acceleration_stack       = prctile(spect_t_acceleration(signal_in_range), 0:5:100);

    speed_play_matrix            = nan(numel(speed_stack),6);
    acceleration_play_matrix     = nan(numel(speed_stack),6);

    for j=1:numel(speed_stack)-1
        data_play   = mean(log10(pow_spectrogram(f_index,find(spect_t_speed>=speed_stack(j) & spect_t_speed<=speed_stack(j+1) & play_bout_spect_time==1 & signal_in_range))), 'omitmissing');
        data_noplay = mean(log10(pow_spectrogram(f_index,find(spect_t_speed>=speed_stack(j) & spect_t_speed<=speed_stack(j+1) & play_bout_spect_time==0 & signal_in_range))), 'omitmissing');
    ...
    if ch_n==1
        stack_lm_pow_structure.speed_stack = speed_stack;
        stack_lm_pow_structure.acceleration_stack = acceleration_stack;
        stack_lm_pow_structure.speed_play_matrix = speed_play_matrix;
        stack_lm_pow_structure.acceleration_play_matrix = acceleration_play_matrix;
    else

        stack_lm_pow_structure(ch_n).speed_stack = speed_stack;
        stack_lm_pow_structure(ch_n).acceleration_stack = acceleration_stack;
        stack_lm_pow_structure(ch_n).speed_play_matrix = speed_play_matrix;
        stack_lm_pow_structure(ch_n).acceleration_play_matrix = acceleration_play_matrix;
    end
    ...
    if ch_n==1
        stack_lm_pow_structure.lm_table = lm_table;
        stack_lm_pow_structure.lm_speed_play = lm_speed_play;

    else

        stack_lm_pow_structure(ch_n).lm_table = lm_table;
        stack_lm_pow_structure(ch_n).lm_speed_play = lm_speed_play;
