...
f          = {.1:.1:5, 5:.1:16};      % frequency range for spectrogram
freq_range = {[.1 6],[6 12]}; 
wind_length = {1, .250};
...
[sorted_play_bout_lengths, order] = sort(play_bout_lengths);

% 
% 
pow_matrix_low_Freq = stacked_data.play_bout_onset_high_freq;
...
for j=1:size(pow_matrix_low_Freq,1)
    ...
    pow_matrix_low_Freq(j,:)  = movmean( pow_matrix_low_Freq(j,:),.125/bin_size, 'omitmissing');
    pow_matrix_low_Freq(j,time>play_bout_lengths(j)) = NaN;
end
...
for j=1:size(pow_matrix_low_Freq,1)
    pow_matrix_high_Freq(j,:) = (pow_matrix_high_Freq(j,:) - mean(pow_matrix_high_Freq(j, baseline_index), 'omitmissing'))/std(pow_matrix_low_Freq(j, baseline_index), 'omitmissing');
...
regressor_table = table(call_regressor, other_speed_regressro,self_speed_regressor,other_acc_regressro,slef_acc_regressro, ...
    ...
    self_angle_speed_regressor, self_angle_acc_regressor, ...
    play_bout,animal_name(:),channel_num(:), animal_dist_regressro,pow_regressor, ...
    'VariableNames', {'Call', 'OtherSpeed','SelfSpeed','OtherAcc','SelfAcc', ...
    'SelfBool', 'OtherBool','OtherAngleSpeed','OtherAngleAccc','RelativeSpeed', ...
    'SelfAngleSpeed','SelfAngleAccc', ...
...
%% plot effect of animal dist
distVals = linspace(min(regressor_table.AnimalDist), max(regressor_table.AnimalDist), 100)';
n_subsamples = 1000;
all_y_hat = nan(k,numel(distVals) );
all_subsample_data =nan(n_subsamples*k,3);
figure
for fold = 1:k
    trainIdx = training(cv, fold);
    testIdx = test(cv, fold);
    ...
    T = trainData;
newT = repmat(T(1,:), 100, 1); % copy structure
newT.AnimalDist = distVals;

% Hold other predictors at their mean/median
predictors = {'Call','OtherSpeed','SelfSpeed','OtherAcc','SelfAcc','SelfBool','OtherBool',...
              'OtherAngleSpeed','OtherAngleAccc','RelativeSpeed','SelfAngleSpeed','SelfAngleAccc',...
              'PlayBout'};
for p = 1:numel(predictors)
    if islogical(newT.(predictors{p})) || all(ismember(unique(newT.(predictors{p})),[0 1]))
        newT.(predictors{p}) = repmat(mode(T.(predictors{p})),size(newT,1),1);
    else
        newT.(predictors{p}) = repmat(mean(T.(predictors{p})),size(newT,1),1);
    end
end

yhat = predict(fullModel, newT);
all_y_hat(fold,:) = yhat;
...
sub_sample = randsample(size(trainData,1),n_subsamples);
 h = plot(trainData.AnimalDist(sub_sample), trainData.Power(sub_sample), '.');
 hold on
 all_subsample_data(((k-1)*n_subsamples)+1:k*n_subsamples,:) = [trainData.AnimalDist(sub_sample) trainData.Power(sub_sample) repmat(fold,n_subsamples,1)];
 plot(distVals, yhat, '-', 'LineWidth', 2, 'Color', h.Color);


pause(.1)
end

xlabel('Animal distance');
ylabel('Predicted Theta power');
title('Model-predicted effect of AnimalDist');