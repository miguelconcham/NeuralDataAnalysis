...
all_lengths = diff(stacked_data.play_bouts_table');
...
play_bout_lengths(j,1) = sum(all_lengths(categorical(stacked_data.meta(:,1))==animal_lst(j) & cell2mat(stacked_data.meta(:,2))==1));
play_bout_lengths(j,2) = sum(all_lengths(categorical(stacked_data.meta(:,1))==animal_lst(j) & cell2mat(stacked_data.meta(:,2))==2));
end
...
subplot(2,1,2)
plot(play_bout_lengths')

%%

figure
plot(diff(play_bout_lengths'),diff(mean_delta_per_animal'),'.')
