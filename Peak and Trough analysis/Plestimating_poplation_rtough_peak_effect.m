...
for j=1:numel(what2plot)
    subplot(3,6,j)
    com_psth_results_means = squeeze(mean(com_psth_results(:,what2plot(j),types2plot(1),:),2, 'omitmissing'));
...
com_psth_results_means = squeeze(mean(com_psth_results(:,what2plot(j),types2plot(2),:),2, 'omitmissing'));
pctl2plot =  prctile(com_psth_results_means,[5 95]);
